...
def run_windows_coverage():
    ...
    if not test_exes:
        test_exes = glob.glob(
            os.path.join(BUILD_DIR, "**", "*_test.exe"), recursive=True
        )
...
def run_linux_coverage():
    ...
    run_command(
        f"lcov --directory {BUILD_DIR} --zerocounters", "Failed to reset counters"
    )
    run_command(
        f"cd {BUILD_DIR} && ctest --output-on-failure",
        "Tests failed! Coverage aborted.",
    )
    run_command(
        f"lcov --directory {BUILD_DIR} --capture --output-file {INFO_FILE} --ignore-errors mismatch,source",
        "Failed to capture",
    )
...
def main():
    if not os.path.exists(BUILD_DIR):
        print(
            f"[Error] Build directory '{BUILD_DIR}' does not exist. Run build.py first."
        )
        sys.exit(1)
